...
function init() {
    ...
    areaTable = $('#datatable').dataTable({
        "ajax": {
            ...
            "data": function (data) {
                //添加额外的参数传给服务器
                data.appName="timetableInfo_compuls";
...
function addCompuls() {
    ...
    if(classId == null || classId.length ==0){
        informationAlert_OnlyConfirmButton_NOT_REFRESH('班级必选');
        return;
    }
    ...
    msg.classId=classId;
    msg.className=$("#classId").find("option:selected").text();

    msg.appName="timetableInfo_compulsAdd";
