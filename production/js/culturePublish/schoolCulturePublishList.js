...
function initClsPublishCulture() {
    areaTable = $('#datatable').dataTable({
        "ajax": {
            ...
            "data": function (data) {
                //添加额外的参数传给服务器
                data.appName="cultures_cultureList";
...
function updateClsPublishCulture(cultureId) {
    ...
    msg.appName="cultures_cultureById";
...
function updateClick() {
    ...
    msg.appName="cultures_cultureUpdate";
    msg.title=$("#title").val();
...
function deleteClsPublishCulture(cultureId) {
    ...
    msg.appName="cultures_cultureDeleted";
    msg.cultureId=cultureId;
    serverFromJSONData(msg,true).then(function (success) {
...
function getCultureInfo(cultureId) {
    ...
    serverFromJSONData(msg,true).then(function (success) {
        ...
        $("#createClass1").html("发布学校："+success.data.schoolName);
        ...
        $("#vedio1find").html("<a href='#' data-toggle='modal' data-target='#myModalsPicvedio' onclick='playVedio(&quot;"+ success.data.videoUrl + "&quot;)'>点击查看</a>");
        
        if(success.data.videoUrl==null){
        	document.getElementById("findVideoSchool").style.display="none";
        }else{
        	document.getElementById("findVideoSchool").style.display="inline";
        };
