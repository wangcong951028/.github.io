...
$(function () {
	 //进行主题重复的验证
      $("#titleAdd").blur(function(){
	  		//发送ajax请求
	  		var msg = {};
		    msg.appName = "cultures_culturesValidate";
		    ...
		    $.ajax({
		        ...
		        success: function (success) {
		            if(success.data != null){
		            		informationAlert_OnlyConfirmButton_NOT_REFRESH('主题名不能重复');
		            } 
		        },
...
function publicCulture() {
    ...
    if($("#intervalTimeAdd").val() == null || $("#intervalTimeAdd").val().length ==0){
    ...
    var msg = {};
    msg.appName="cultures_cultureAdd";
    msg.title=$("#titleAdd").val();
...
function AddCulture(msg) {
    ...
    serverFromJSONData(msg,true).then(function (success) {
        if(success.msgState==200){
            ...
            informationAlert_OnlyConfirmButton_NOT_REFRESH("创建文化展示失败，原因："+success.msg);
        }
        refresh();
    }),function (error) {
...
function uploadUser(){
    ...
    serverFromJSONData(param,true).then(function (response) {
        ...
        if (response.msgState == 200){
            ...
            $("#wrapper").show();
            var rsp=uploadFile("/apis/SC/"+param.interfaceName);
        }else{
